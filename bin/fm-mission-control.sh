...
# task board, a card per project, the decisions waiting on the captain, a week
# calendar, the crew as an org chart, a reader for the crew's memory and
# documents, and the health of the machine and the crew's plumbing, in one
# terminal pane; bin/fm_mission_control.py owns what it reads, how the crew
# maps onto the office, and how frames are drawn. It is read-only: it never
# writes a record, never sends keys or prompts to an agent, and never starts
# or stops one.
#
# Commands:
