...
# calendar week (completions, due dates, what always runs), the Team org
# chart, and the Memory and Docs lists with their Markdown reader; then the
# live screen in a pseudo-terminal, which must redraw only what changed, pick
# project cards with up/down, pick, filter and scroll Memory and Docs pages,
# and restore the terminal on q and on SIGTERM. The record parsers themselves are covered by fm-bridge.test.sh.
